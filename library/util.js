...
module.exports.sendAttendanceSMS = async (className, stuName, attendTime, phoneNum) => {
    ...
    //   const { tel } = req.body;
      ...
      hmac.update(space);
      hmac.update(url2);
      hmac.update(newLine);// 이거 둘다 있어야 함
      // console.log( "뉴라인1",newLine)
      hmac.update(date);
      hmac.update(newLine);// 이거 둘다 있어야 함
      // console.log( "뉴라인2",hmac.update(newLine))
      console.log(sens_access_key);
      hmac.update(sens_access_key);
      const hash = hmac.finalize();
      console.log(4);
      const signature = hash.toString(CryptoJS.enc.Base64); // 한글깨짐 방지 인코딩/디코딩
      console.log(5);
...
//       //HMAC 알고리즘 사용 암호화 - SHA256 : 여러개의 공유 메시지 사용
//       const hmac = CryptoJS.algo.HMAC.create(CryptoJS.algo.SHA256, sens_secret_key);
